...
DEFAULT_DELIVERY_FIELD_NAMES = ["ALM_Delivery", "Delivery", "Release", "Target Release", "Fix Version", "Sprint"]
DEFAULT_DELIVERY_DATE_FIELD_NAMES = ["Target Date", "Delivery Date", "Due Date", "Planned End Date", "End Date"]
DEFAULT_REMAINING_FIELD_NAMES = ["Remaining", "Remaining Items", "Remaining Estimate", "Remaining Effort", "Story Points"]
DEFAULT_BLOCKED_FIELD_NAMES = ["Blocked", "Is Blocked", "Blocker"]
...
class CodebeamerConfig:
    ...
    delivery_fields: List[str] = None
    delivery_date_fields: List[str] = None
    remaining_fields: List[str] = None
    ...
    def from_env() -> "CodebeamerConfig":
        ...
        return CodebeamerConfig(
            ...
            delivery_fields=_csv_env("CB_DELIVERY_FIELDS", DEFAULT_DELIVERY_FIELD_NAMES),
            delivery_date_fields=_csv_env("CB_DELIVERY_DATE_FIELDS", DEFAULT_DELIVERY_DATE_FIELD_NAMES),
            remaining_fields=_csv_env("CB_REMAINING_FIELDS", DEFAULT_REMAINING_FIELD_NAMES),
            blocked_fields=_csv_env("CB_BLOCKED_FIELDS", DEFAULT_BLOCKED_FIELD_NAMES),
        )
...
class CodebeamerClient:
    ...
    async def query_items(self, query: str, page_size: Optional[int] = None, max_pages: Optional[int] = None) -> Tuple[List[Dict[str, Any]], int]:
        ...
        for page in range(1, max_pages + 1):
            payload = {"page": page, "pageSize": page_size, "queryString": query}
            data = await self.post("/api/v3/items/query", payload)
            batch = data.get("items") or []
            total = int(data.get("total") or total or len(batch))
            all_items.extend(batch)

            if not batch or len(batch) < page_size or (total and len(all_items) >= total):
                break
...
def _names(value: Any) -> List[str]:
    ...
    if isinstance(value, list):
        return [n for n in (_name(v) for v in value) if n]
    n = _name(value)
    return [n] if n else []
...
def _custom_fields(fields: Any) -> Dict[str, Any]:
    ...
    for field in fields:
        ...
        if "value" in field:
            ...
            result[key] = _names(field.get("values"))
        else:
            result[key] = None
    return result
...
def _boolish(value: Any) -> bool:
    ...
    if isinstance(value, (int, float)):
        return value != 0
    return str(value).strip().lower() in {"true", "yes", "1", "blocked", "blocker"}
...
def normalize_item(item: Dict[str, Any], cfg: CodebeamerConfig) -> Dict[str, Any]:
    ...
    delivery_raw = (
        ...
        or item.get("release")
        or item.get("version")
        or (_names(item.get("versions"))[0] if _names(item.get("versions")) else None)
        or "Unassigned"
    )
    delivery_name = delivery_raw[0] if isinstance(delivery_raw, list) and delivery_raw else _name(delivery_raw) or "Unassigned"
    ...
    return {
        "id": item_id,
        "title": item.get("name") or item.get("title") or item.get("summary") or f"Item {item_id}",
...
def build_deliveries(items: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    for item in items:
        ...
        delivery = grouped.setdefault(key, {
            "id": key,
...
def build_dashboard(items: List[Dict[str, Any]], query: str, total_reported: int) -> Dict[str, Any]:
    ...
    return {
        ...
        "totals": {
            ...
            "atRisk": sum(1 for item in open_items if item.get("blocked") or item.get("priority", "").lower() in HIGH_PRIORITIES),
        },
        "breakdowns": {
...
def build_receipt(tool: str, input_payload: Dict[str, Any], output_summary: Dict[str, Any], elapsed_ms: int) -> Dict[str, Any]:
    return {
        "receipt_id": f"rcpt_{datetime.now(timezone.utc).strftime('%Y%m%d%H%M%S')}_{uuid.uuid4().hex[:8]}",
        "tool": tool,
        "source": "ptc-codebeamer",
        "authority": "non_authoritative",
        "receipt_backed": True,
...
async def cdyp7_cb_projects() -> Dict[str, Any]:
    """List accessible PTC Codebeamer projects for the configured service account."""
    started = time.time()
    ...
    try:
        ...
        result = [{"id": p.get("id"), "name": p.get("name")} for p in projects_list if isinstance(p, dict)]
        elapsed = int((time.time() - started) * 1000)
        return {
            "status": "ok",
            "authority": "non_authoritative",
            "receipt": build_receipt("cdyp7.cb.projects", {}, {"projects": len(result)}, elapsed),
            "projects": result,
        }
    finally:
        await client.close()
...
@mcp.tool()
async def cdyp7_cb_query_items(query: str = "status != 'Closed'", page_size: int = 100, max_pages: int = 10) -> Dict[str, Any]:
    """Query PTC Codebeamer tracker items and return normalized ALM items."""
    started = time.time()
    client = await _client()
    cfg = client.config
    try:
        raw, total = await client.query_items(query, page_size=page_size, max_pages=max_pages)
        items = [normalize_item(item, cfg) for item in raw]
        elapsed = int((time.time() - started) * 1000)
        receipt = build_receipt("cdyp7.cb.query_items", {"query": query, "page_size": page_size, "max_pages": max_pages}, {"items": len(items), "totalReported": total}, elapsed)
        return {
            "status": "ok",
            "authority": "non_authoritative",
            "receiptBacked": True,
...
@mcp.tool()
async def cdyp7_cb_dashboard(query: str = "status != 'Closed'", page_size: int = 100, max_pages: int = 25, persist: bool = True) -> Dict[str, Any]:
    """
    Build the CDYP7 dashboard state from PTC Codebeamer.

    Uses ALM_Delivery as the delivery string field and Target Date as the delivery date
    when those fields are present in Codebeamer customFields.
    """
    started = time.time()
    client = await _client()
    cfg = client.config
    try:
        raw, total = await client.query_items(query, page_size=page_size, max_pages=max_pages)
        normalized = [normalize_item(item, cfg) for item in raw]
        dashboard = build_dashboard(normalized, query, total)
        elapsed = int((time.time() - started) * 1000)
        receipt = build_receipt(
            "cdyp7.cb.dashboard",
            {"query": query, "page_size": page_size, "max_pages": max_pages, "persist": persist},
            {"items": dashboard["totals"]["items"], "openItems": dashboard["totals"]["openItems"], "deliveries": len(dashboard["deliveries"])},
            elapsed,
        )
...
def dashboard_triage_prompt() -> str:
    ...
    return (
        ...
        "Focus on open items, ALM_Delivery grouping, Target Date risk, assignedTo ownership load, "
        "blocked items, and high-priority items. Summarize risks and recommended follow-up actions."
    )


if __name__ == "__main__":
    transport = os.getenv("MCP_TRANSPORT", "stdio")
    # For browser/gateway use, set MCP_TRANSPORT=streamable-http.
    # With the official SDK, this normally serves at http://localhost:8000/mcp.
